...
class IG(TargetedAttacker):
    ...
    def attack(self, target, n_perturbations=None, direct_attack=True,
        ...
        if structure_attack and not feature_attack:
            ...
            self.feature_flips = candidate_features[largest_indices(feature_importance, self.n_perturbations)[0]]
        else:
            ...
            if feature_selected:
                self.feature_flips = candidate_features[feature_selected]
    ...
    def get_candidate_features(self):
        ...
        if self.direct_attack:
            influence_nodes = [target]
            candidate_features = np.column_stack((np.tile(target, n_attrs), self.features_set))
        else:
            # influence_nodes = self.adj[target].nonzero()[1]
            influence_nodes = self.adj[target].indices
            candidate_features = np.row_stack([np.column_stack((np.tile(infl, n_attrs), self.features_set)) for infl in influence_nodes])
