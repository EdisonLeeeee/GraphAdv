...
from graphadv.utils.graph_utils import filter_singletons
from graphadv.attack.targeted.targeted_attacker import TargetedAttacker
from graphgallery.nn.models import GCN
from graphgallery import tqdm, normalize_adj
...
class Nettack(TargetedAttacker):
    ...
    def compute_logits(self):
        ...
        Returns
        -------
        np.array, [n_nodes, n_classes]
            The log probabilities for each node.

        """
        return (self.adj_norm @ self.adj_norm @ self.modified_x @ self.W)[self.target].ravel()
    ...
    def strongest_wrong_class(self, logits):
        ...
        Parameters
        ----------
        logits: np.array, [n_nodes, n_classes]
            The input logits
    ...
    def struct_score(self, a_hat_uv, XW):
        ...
        XW: np.array, shape [n_nodes, n_classes], dtype float
        ...
        logits = a_hat_uv @ XW
        label_onehot = np.eye(self.n_classes)[self.target_label]
    ...
    def compute_XW(self):
        ...
        Returns
        -------
        x @ W: np.array, shape [n_nodes, n_classes]
        """

        return self.modified_x @ self.W
...
#         assert self.target not in neighbors
        ...
        if len(neighbors) >= n:  # do we have enough neighbors for the number of desired influencers?
            ...
            if add_additional_nodes:  # Add additional influencers by connecting them to u first.
                ...
                poss_add_infl = np.setdiff1d(np.setdiff1d(np.arange(self.n_nodes), neighbors), self.target)
    ...
    def compute_new_a_hat_uv(self, potential_edges):
        ...
        # edges_set = set(self.graph.edges())
        ...
        with warnings.catch_warnings(record=True):
            ...
            ixs, vals = compute_new_a_hat_uv(edges, node_ixs, edges_set, twohop_ixs, values_before, degrees,
                                             potential_edges, self.target)
        ixs_arr = np.array(ixs)
        a_hat_uv = sp.coo_matrix((vals, (ixs_arr[:, 0], ixs_arr[:, 1])), shape=[len(potential_edges), self.n_nodes])

        return a_hat_uv

    def attack(self, target, n_perturbations=None, direct_attack=True,
               structure_attack=True, feature_attack=False,
               n_influencers=5, ll_constraint=True, ll_cutoff=0.004, disable=False):
        ...
        for _ in tqdm(range(self.n_perturbations), desc='Peturbing Graph', disable=disable):
            if structure_attack:
                ...
                if not self.allow_singleton:
                    filtered_edges = filter_singletons(self.potential_edges, self.modified_adj)
                else:
...
def compute_new_a_hat_uv(edge_ixs, node_nb_ixs, edges_set, twohop_ixs, values_before, degs, potential_edges, u):
    ...
    node_nb_ixs: np.array, shape [n_nodes,], dtype int
    ...
    values_before: np.array, shape [n_nodes,], the values in [A_hat]^2_uv to be updated.
    degs: np.array, shape [n_nodes,], dtype int
    ...
    return_ixs: List of tuples
        The ixs in the [P, n_nodes] matrix of updated values that have changed
    return_values:
    ...
    for ix in range(len(potential_edges)):
        ...
        for v in affected_nodes:
            ...
            to_ix = node_nb_ixs[v + 1] if v < n_nodes - 1 else len(edge_ixs)
...
def update_Sx(S_old, n_old, d_old, d_new, d_min):
    ...
    d_old: np.array, shape [n_nodes,] dtype int
        The old degree sequence.

    d_new: np.array, shape [n_nodes,] dtype int
